...
- **手弾きベース**: コードボタンを押すたびにルートを鳴らし、離すと止まる。奏法は なし / ルート / ルート+オクターブ / オクターブ交互、**ベースオクターブ** (-1〜+1)、**ベース音色** は GM 33〜40 のベース系 + Square / Saw
...
## 設定の保存
MIDI 出力 / 入力ポート、楽器、キー、テンポ、オクターブ、アルペジオ番号、ストローク / 自動アルペジオ / 自動ベースの選択、ベース音色 / 手弾きベース / ベースオクターブ、ドラムパターンは `localStorage` に保存され、次回開いたときに復元される。MIDI ポートは後から接続されても保存したものが見つかり次第選び直す。
