...
- **オンコード**: ベース音を ルート / 3度 / 5度 / ♭7度 / キーの 1〜7 度 に変える (C/E など)。コード名にも `/E` と出て、手弾き・自動ベースの両方に効く
- **手弾きベース**: コードボタンを押すたびにルート (オンコードならその音) を鳴らし、離すと止まる。奏法は なし / ルート / ルート+オクターブ / オクターブ交互、**ベースオクターブ** (-1〜+1)、**ベース音色** は GM 33〜40 のベース系 + Square / Saw
- **ドラム** ボタン: 手弾きのままドラムパターン (Ballad / 8beat / 16beat / Waltz / Bossa / Rock) だけを回す。自動演奏に切り替えるとそのパターンのドラムが鳴る
...
- **MIDI 出力**: セレクトで PC 上の MIDI 音源 (IAC Driver 経由の DAW / ソフトシンセなど) を選ぶと内蔵音源の代わりに送る
  - コード: 6 弦を ch1〜6 に 1 弦 1 チャンネルで送る (6ch とも楽器の GM プログラムチェンジ)、ベース: ch7 (音色は「ベース音色」で選択)、ドラム: ch10 (36 kick / 38 snare / 42 hi-hat)
...
## 設定の保存
MIDI 出力 / 入力ポート、楽器、キー、テンポ、オクターブ、アルペジオ番号、ストローク / 自動アルペジオ / 自動ベースの選択、ベース音色 / 手弾きベース / オンコード / ベースオクターブ、ドラムパターンは `localStorage` に保存され、次回開いたときに復元される。MIDI ポートは後から接続されても保存したものが見つかり次第選び直す。
